...
useEffect(() => {
    ...
    const userEmail = currentUser?.email; // Safely access email using optional chaining
  setUser( userEmail ) // Retrieve user data using userEmail if it exists
  ...
  // In the JSX, check if user exists and display relevant information
  return (
...
<h1 style={{ color: 'rgb(246,164,31)', margin: '0', padding: '10px', textAlign: 'center', fontSize: '2.5rem', fontWeight: 'bold', backgroundColor: '#f0f0f0', borderRadius: '10px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
  Weather App
  <span style={{ fontSize: '1rem', fontStyle: 'italic', marginLeft: '10px', verticalAlign: 'sub' }}>powered by company</span>
</h1>
      <div className="absolute top-0 right-0 m-4 mr-9 group">
        ...
        <div className="p-2 border-b">{user ? user : 'Guest'} {/* Replace with dynamic username if available */}</div>
      ...
      <div className="mt-4 p-4 bg-white rounded-lg shadow-md flex flex-wrap gap-4 w-fit mx-auto" style={{ minWidth: '20rem', minHeight: '20rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <br></br>
